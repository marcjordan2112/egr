...
# _DEFAULT_URL_ESOUI_API_TXT_DOCUMENTATION = "https://github.com/esoui/esoui/blob/master/ESOUIDocumentation.txt"
...
def process_esoui_api_txt_documentation(
    ...
    if False:
        blah = []
        for count, these_constants in enumerate(global_constants_list):
            blurp = {"constant_label": these_constants[0]["constant_label"]}
            bleep = ["wut"] * len(these_constants)
            for this_constant in these_constants:
                bleep[live_constants_from_dumpvars[this_constant["constant_name"]]] = \
    ...
    with pathlib.Path(_ESOUI_CONSTANTS_LIVE_LUA_FILE).open('wb') as f:
        ...
        for these_constants in global_constants_list:
            ...
            these_constants_sorted_by_index = sorted(these_constants, key=lambda d: d['index'])
            pass
            for count, this_constant in enumerate(these_constants_sorted_by_index):
                # _logger.debug("    %s" % this_constant["constant_name"])
                # _logger.debug("    %d" % this_constant["index"])
